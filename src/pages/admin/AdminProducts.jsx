...
// 👇 O Dicionário que resolve o erro do "never[]" e do "id"
/**
...
export default function AdminProducts() {
  ...
  const addTag = () => {
    if (tagInput.trim() && !form.tags?.includes(tagInput.trim())) {
      setForm(/** @type {any} */ (prev) => ({ ...prev, tags: [...(prev.tags || []), tagInput.trim()] }));
      setTagInput('');
    }
  };

  /** @param {string} id */
  const getCategoryName = (id) => categories.find((/** @type {any} */ c) => c.id === id)?.name || '—';
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><Label>Slug</Label><Input value={form.slug} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, slug: e.target.value }))} /></div>
              <div><Label>Preço (R$) *</Label><Input type="number" step="0.01" value={form.price} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, price: e.target.value }))} required /></div>
              <div><Label>Preço Original (R$)</Label><Input type="number" step="0.01" value={form.compare_price} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, compare_price: e.target.value }))} /></div>
              <div>
              ...
              <div><Label>Cor</Label><Input value={form.color} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, color: e.target.value }))} /></div>
              <div><Label>Dimensões</Label><Input value={form.dimensions} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, dimensions: e.target.value }))} placeholder="ex: 10x5x3 cm" /></div>
              <div><Label>Peso</Label><Input value={form.weight} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, weight: e.target.value }))} placeholder="ex: 50g" /></div>
              ...
              <div><Label>Preenchimento</Label><Input value={form.infill} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, infill: e.target.value }))} placeholder="ex: 20%" /></div>
            </div>

            <div><Label>Descrição Curta</Label><Textarea value={form.short_description} onChange={e => setForm(/** @type {any} */ (p) => ({ ...p, short_description: e.target.value }))} rows={2} /></div>
            ...
            <div>
              ...
              {form.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {form.tags.map((t, i) => (
                    <Badge key={i} variant="secondary" className="cursor-pointer" onClick={() => setForm(/** @type {any} */ (p) => ({ ...p, tags: p.tags.filter((/** @type {any} */ _, /** @type {number} */ j) => j !== i) }))}>
                      {t} ×
                    </Badge>
                  ))}
                </div>
